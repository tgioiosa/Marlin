...
//TG - 10/2/21 added to slow down TEMP_TIMER ISR (and RPM_tick calls) to 4ms if logging to serial terminal
//     to avoid clobbering those interrupts due to serial logging taking up to 3ms each call.
//     This change must occur before RPM_timer_init() and thermalManager.init() since they rely on it.
//     That's why it's put here in Conditionals_post.h to get evaluated early enough.
#if ANY(FAST_RPM_REPORTING, PID_WAVEFORM_LOGGING)
  #undef TEMP_TIMER_FREQUENCY
  #define TEMP_TIMER_FREQUENCY 250
#endif
